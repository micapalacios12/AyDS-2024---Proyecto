...
RSpec.describe 'App Routes', type: :request do
  ...
  context 'POST /register' do
    it 'registers a new user and redirects to login' do
      post '/register', names: 'John Doe', username: 'johndoe', email: 'john@example.com', password: 'password123',
    ...
    it 'fails to register when the username is already taken' do
      ...
      post '/register', names: 'Jane Doe', username: 'johndoe', email: 'jane@example.com', password: 'password123',
                        password_confirmation: 'password123'
    ...
    it 'fails to register when the email is already registered' do
      ...
      post '/register', names: 'Jane Doe', username: 'janedoe', email: 'john@example.com', password: 'password123',
                        password_confirmation: 'password123'
  ...
  context 'POST /configuracion' do
    it 'updates user settings and redirects to profile' do
      ...
      post '/configuracion', names: 'New Name', username: 'newusername', email: 'new@example.com',
                             avatar: 'new_avatar.png', password: 'newpassword', password_confirmation: 'newpassword'
      expect(last_response).to be_redirect
      follow_redirect!
      expect(last_request.path).to eq('/profile')
  ...
  context 'POST /play/question' do
    ...
    it 'correctly processes a valid correct answer' do
      post '/play/question', { option_id: correct_option.id },
           'rack.session' => { user_id: user.id, current_question_index: 0, system: 'digestivo', level: 1 }
    ...
    it 'shows an error message for an invalid option' do
      post '/play/question', { option_id: 999 },
           'rack.session' => { user_id: user.id, current_question_index: 0, system: 'digestivo', level: 1 }
    ...
    it 'redirects to /finish_play if there are no more questions' do
      post '/play/question', { option_id: correct_option.id },
           'rack.session' => { user_id: user.id, current_question_index: 1, system: 'digestivo', level: 1 }
